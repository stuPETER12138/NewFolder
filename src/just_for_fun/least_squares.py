...
def plot_data_and_fit(
    x, y, slope, intercept, sensitivity, nonlinearity_error, output_path=None
):
    """
    绘制数据点和拟合直线
    ...
    # 添加标题和标签
    plt.title(
        f"最小二乘法拟合结果 (灵敏度 = {sensitivity:.4f}, 非线性误差 = {nonlinearity_error:.2f}%)"
    )
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.grid(True)
...
def process_data_file(file_path, output_dir=None):
    ...
    # 绘制结果
    plot_data_and_fit(
        x, y, slope, intercept, sensitivity, nonlinearity_error, output_path
    )
...
def main():
    ...
    parser.add_argument("--no-plot", action="store_true", help="不显示图像 (仅保存)")
